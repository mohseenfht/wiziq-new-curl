			 ...
			 ?>
				<table class="list_courses sortable" id="list_courses" >
					...
					if($courses){
						...
						$Wiziq_Util = new Wiziq_Util;						
						foreach($wiziq_course_res as $course){
                /* start custom code to show only those courses in whiich he enrolled */                                                      
                               /*                     $current_user_id = get_current_user_id();
                                                    $courseid=$course->id;
                            $wiziq_enroluser = $wpdb->prefix."wiziq_enroluser";
                             $request="select * from $wiziq_enroluser where user_id = $current_user_id && course_id=$courseid " ;
                            $result_is_user_enrolled_to_course = $wpdb->get_results($request); 
                           echo "<pre>";
                           print_r($result_is_user_enrolled_to_course);
                           if(!empty($result_is_user_enrolled_to_course)){ */
               /* end custom code to show only those courses in whiich he enrolled */                  
